---
name: init
description: Initialize this Tauri + React template for a specific application.
---
...
5. **Update `AGENTS.md`** (and `CLAUDE.md` if this repo still uses it) to reflect the app name/description.
6. **Update `README.md`** to replace template naming with the app name and update the description.
7. **Update `src-tauri/tauri.conf.json`**:
   ...
   - Set window `title` to the app name.
   - Update bundle `shortDescription` and `longDescription`.
   - Update bundle `publisher` and `copyright` with GitHub username.
   - Update updater endpoint to use the GitHub username and repo name.
8. **Update `src-tauri/Cargo.toml`**:
   - Set package `name` to kebab-case app name.
   - Set package `description` to the description.
   - Update `authors` with GitHub username.
9. **Update `.github/workflows/release.yml`** (if present):
   - Update workflow name to use the app name.
   - Update release name and body to use the app name.
